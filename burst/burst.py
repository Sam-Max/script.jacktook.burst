...
def search(payload, method="general"):
    ...
    if method == "general":
        if "query" in payload:
            ...
            payload = {
                "title": payload,
                "titles": {"source": payload, "original": payload},
            }
    ...
    payload["has_special"] = (
        "titles" in payload
        and bool(payload["titles"])
        and "source" in payload["titles"]
        and any(c in payload["titles"]["source"] for c in special_chars)
    )
    ...
    for provider in providers:
        ...
        task = Thread(
            target=run_provider,
            args=(provider, payload, method, providers_time, timeout),
        )
        task.start()
    ...
    while time.time() - providers_time < timeout and available_providers > 0:
        ...
        if timer > timeout:
            break
        message = (
            translation(32062) % available_providers
            if available_providers > 1
            else translation(32063)
        )
        if not payload["silent"]:
            p_dialog.update(
                int((total - available_providers) / total * 100), message=message
            )
        time.sleep(0.25)
    ...
    logging.debug(
        "all provider_results of %d: %s"
        % (len(provider_results), repr(provider_results))
    )

    filtered_results = apply_filters(provider_results)

    logging.debug(
        "all filtered_results of %d: %s"
        % (len(filtered_results), repr(filtered_results))
    )

    logging.info(
        "Providers returned %d results in %s seconds"
...
def got_results(provider, results):
    ...
    if not sort_by or sort_by == 3 or sort_by > 3:
        ...
        sorted_results = sorted(
            results, key=lambda r: (r["sort_balance"]), reverse=True
        )
    elif sort_by == 0:
        sorted_results = sorted(
            results, key=lambda r: (r["sort_resolution"]), reverse=True
        )
    elif sort_by == 1:
        sorted_results = sorted(results, key=lambda r: (r["seeds"]), reverse=True)
    elif sort_by == 2:
        sorted_results = sorted(
            results, key=lambda r: (size_int(r["size"])), reverse=True
        )
    ...
    logging.info(
        "[%s] >> %s returned %2d results in %.1f seconds%s"
        % (
            provider,
            definition["name"].rjust(longest),
            len(results),
            round(time.time() - request_time, 2),
            (
                (", sending %d best ones" % max_results)
...
def extract_torrents(provider, client):
    ...
    definition = get_alias(definition, get_setting("%s_alias" % provider))
    logging.debug(
        "[%s] Extracting torrents from %s using definitions: %s"
    ...
    if needs_subpage:
        ...
        def extract_subpage(q, name, torrent, size, seeds, peers, info_hash, referer):
            try:
                ...
                logging.error(
                    "[%s] Subpage loggingging failed with: %s" % (provider, repr(e))
                )
            ...
            if "bittorrent" in subclient.headers.get("content-type", ""):
                logging.debug(
                    "[%s] bittorrent content-type for %s" % (provider, repr(torrent))
                )
                ...
                try:
                    ...
                    if (
                        torrent and not torrent.startswith("magnet") and len(uri) > 1
                    ):  # Stick back cookies if needed
                        torrent = "%s|%s" % (torrent, uri[1])
                except Exception as e:
                    import traceback
            ...
            logging.debug(
                "[%s] Subpage torrent for %s: %s" % (provider, repr(uri[0]), torrent)
            )
    ...
    if not dom:
        if debug_parser:
            logging.debug(
                "[%s] Parser debug | Could not parse DOM from page content" % provider
            )
    ...
    if debug_parser:
        logging.debug(
            "[%s] Parser debug | Matched %d items for '%s' query '%s'"
    ...
    for item in items:
        ...
        try:
            ...
            if debug_parser:
                ...
                logging.debug(
                    "[%s] Parser debug | Matched '%s' iteration for query '%s': %s"
                    % (provider, "description", description_search, description)
                )
                logging.debug(
                    "[%s] Parser debug | Matched '%s' iteration for query '%s': %s"
                ...
                if referer_search:
                    logging.debug(
                        "[%s] Parser debug | Matched '%s' iteration for query '%s': %s"
                        % (provider, "referer", referer_search, referer)
                    )
            ...
            if not torrent.startswith("magnet"):
                ...
                if client.passkey:
                    ...
                    logging.debug(
                        "[%s] Appending headers: %s" % (provider, repr(headers))
                    )
                    torrent = append_headers(torrent, headers)
                    logging.debug(
                        "[%s] Torrent with headers: %s" % (provider, repr(torrent))
                    )
                else:
                    ...
                    if cookies:
                        ...
                        headers["Cookie"] = ";".join(
                            ["%s=%s" % (k, v) for (k, v) in iteritems(cookies)]
                        )
            ...
            if name and torrent and needs_subpage and not torrent.startswith("magnet"):
                ...
                if (
                    uri
                    and uri[0]
                    and uri[0] in provider_cache
                    and provider_cache[uri[0]]
                ):
                    yield (name, info_hash, provider_cache[uri[0]], size, seeds, peers)
                    continue

                t = Thread(
                    target=extract_subpage,
                    args=(q, name, torrent, size, seeds, peers, info_hash, referer),
                )
                threads.append(t)
            else:
            ...
            logging.error(
                "[%s] Got an exception while parsing results: %s" % (provider, repr(e))
            )
...
def extract_from_api(provider, client):
    ...
    if (
        "subresults" in api_format
    ):  # A little too specific to YTS/AniLibria but who cares...
    ...
    for result in results:
        ...
        if "name" in api_format:
            name = (
                get_nested_value(result, api_format["name"], "")
                .replace("\n", " ")
                .replace("\r", "")
            )
        ...
        if "quality" in api_format:  # Again quite specific to YTS and AniLibria
            name = "%s - %s" % (
                name,
                get_nested_value(result, api_format["quality"], ""),
            )
    ...
    try:
        ...
        if matches:
            ...
            result = result.replace(
                "xt.1=", "xt="
            )  # libtorrent does not support xt groups
        ...
        if matches:
            ...
            logging.debug(
                "[%s] Matched download link with token: %s" % (provider, repr(result))
            )
            return result
        ...
        if matches:
            result = definition["root_url"] + matches[0]
            logging.debug(
                "[%s] Matched download link with an ID: %s" % (provider, repr(result))
            )
            return result
        ...
        if matches:
            ...
            logging.debug(
                "[%s] Matched magnet info_hash search: %s" % (provider, repr(result))
            )
            return result
        ...
        matches = re.findall(
            r'/engine/download.php\?id=[A-Za-z0-9]+[^\s\'"]*', content
        )  # animaunt
        if matches:
            result = definition["root_url"] + matches[0]
            result += (
                "|Referer=" + result
            )  # we need to add Referer header to download .torrent
            logging.debug("[%s] Matched download link: %s" % (provider, repr(result)))
            return result
...
def cookie_sync(is_silent):
    ...
    if not hasCrypto:
        logging.error(
            "Cryptodome Python module is not available for current Kodi version"
        )
        return
    ...
    p_dialog = xbmcgui.DialogProgressBG()
    if not is_silent:
        p_dialog.create("Jacktook [COLOR FFFF6B00]Burst[/COLOR]", translation(32166))
    ...
    try:
        ...
        for k, v in iteritems(resp_items):
            ...
            try:
                ...
                logging.error("Failed parsing cookies for %s with: %s" % (k, repr(e)))
                import traceback

                map(logging.error, traceback.format_exc().split("\n"))
        logging.debug(
            "Cookie sync fetched for %d domains, %d cookies"
            % (domains_count, cookies_count)
        )
