...
class Client:
    ...
    def __init__(
        self, info=None, request_charset="utf-8", response_charset=None, is_api=False
    ):
        self._counter = 0
        ...
        if proxy["enabled"]:
            ...
            if proxy["use_type"] == 1:
                ...
                logging.debug(
                    "Setting proxy with hosts resolve from Elementum: %s"
                    % (info["proxy_url"])
                )

                proxy_url_scheme_separator = "://"
                elementum_proxy_url_parts = info["proxy_url"].split(
                    proxy_url_scheme_separator
                )
                ...
                if elementum_proxy_url_prefix in elementum_proxy_types_overrides:
                    self.proxy_url = proxy_url_scheme_separator.join(
                        [elementum_proxy_types_overrides[elementum_proxy_url_prefix]]
                        + elementum_proxy_url_parts[1:]
                    )
            elif proxy["use_type"] == 3:
                logging.debug("Setting proxy to Antizapret proxy")
    ...
    def open(self, url, language="en", post_data=None, get_data=None, headers=None):
        ...
        if self.use_antizapret:
            parsed = urlparse(url)
            proxy_res = self.antizapret_proxy.detect(host=parsed.netloc)
            if proxy_res:
                logging.debug(
                    "Detected Antizapret proxy for %s: %s" % (parsed.netloc, proxy_res)
                )
                self.session.proxies = {
                    "http": proxy_res,
                    "https": proxy_res,
