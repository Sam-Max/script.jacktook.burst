...
if not os.access(CACHE_DIR, os.W_OK):
    error_message = (
        "Directory '%s' is not writable, antizapret will not work" % CACHE_DIR
    )
    log.error(error_message)
    ...
    def isInNet(ipaddr, netaddr, netmask):
        ...
        try:
            ...
            net_obj = ipaddress.ip_network(
                "{}/{}".format(netaddr, netmask), strict=False
            )
...
if PY2:  # Python 2 does not have embedded ipaddress module
    ...
    def isInNet(ipaddr, netaddr, netmask):
        ...
        if not ipaddr:  # Handle None from dnsResolve
            return False
        if (
            not _is_ipv4_address(ipaddr)
            or not _is_ipv4_address(netaddr)
            or not _is_ipv4_address(netmask)
        ):
            return False
        ...
        with shelf(CONFIG_FILE_NAME, ttl=CACHE) as pac:
            if not pac.get("value"):
                ...
                try:
                    ...
                    Exception
                ) as e:  # TODO: probably we can re-use stale cache if antizapret pac file is unavailable or blocked.
                    error_message = "Fetching Antizapret PAC file failed: %s" % (
                        repr(e)
                    )
                    log.error(error_message)
                ...
                try:
                    ...
                    domains_groups = re.search(
                        r"domains = \{(.*?)\};", data, flags=re.DOTALL
                    )
                    # dictionary "domains" must have keys in defined order, otherwise unzlp will not work. JavaScript preserves keys order.
                    if (
                        PY3
                    ):  # Python 3 also preserve keys order in dict, so we can simply use eval.
                        domains = eval("{" + domains_groups.group(1) + "}")
                    if (
                        PY2
                    ):  # Python 2 does NOT preserve keys order in dict, so we have to use OrderedDict and parse strings.
                        ...
                        try:
                            ...
                            error_message = "Failed to parse line {}: {}".format(
                                line, e
                            )
                    ...
                    d_ipaddr_groups = re.search(
                        r'var d_ipaddr = "(.*?)"', data, flags=re.DOTALL
                    )
                    d_ipaddr = (
                        d_ipaddr_groups.group(1)
                        .replace("\\", "")
                        .replace("\n", "")
                        .split()
                    )
                    ...
                    special_groups = re.search(
                        r"var special = (\[.*?\]);", data, flags=re.DOTALL
                    )
                    special = eval(special_groups.group(1))

                    # domain name data encoded with LZP, without mask data
                    domains_lzp_groups = re.search(
                        r'var domains_lzp = "(.*?)";', data, flags=re.DOTALL
                    )
                    domains_lzp = (
                        domains_lzp_groups.group(1).replace("\\", "").replace("\n", "")
                    )

                    # LZP mask data, base64+patternreplace
                    mask_lzp_groups = re.search(
                        r'var mask_lzp = "(.*?)";', data, flags=re.DOTALL
                    )
                    mask_lzp = (
                        mask_lzp_groups.group(1).replace("\\", "").replace("\n", "")
                    )

                    # patterns for patternreplace/patternrestore
                    patterns_groups = re.findall(
                        r"var patterns = (\{.*?\});", data, flags=re.DOTALL
                    )
                    ...
                    if (
                        PY2
                    ):  # Python 2 does NOT preserve keys order in dict, so we have to make replacements in string and use OrderedDict.
                        from collections import OrderedDict
                        ...
                        try:
                            ...
                            error_message = "Failed to parse kv {} in patterns_domains_lzp: {}".format(
                                kv, e
                            )
                            log.warning(error_message)
                        ...
                        try:
                            ...
                            error_message = (
                                "Failed to parse kv {} in patterns_mask_lzp: {}".format(
                                    kv, e
                                )
                            )
                            log.warning(error_message)

                    three_part_suffixes_pattern_groups = re.search(
                        r"if \(/(.*?)/.test\(host\)\)", data
                    )
                    three_part_suffixes_pattern = (
                        three_part_suffixes_pattern_groups.group(1)
                    )

                    proxy_groups = re.search(
                        r'return "HTTPS ([^;]*?); PROXY ([^;]*?);', data
                    )  # if we got "proxy-ssl.js" file
                    if proxy_groups:
                        if PY3:
                        ...
                        if (
                            PY2
                        ):  # otherwise Exception: TLS in TLS requires SSLContext.wrap_bio() which isn't supported on Python 2
                            proxy_server = proxy_groups.group(2)
                            proxy_scheme = "http://"
                    else:
                        proxy_groups = re.search(
                            r'return "PROXY ([^;]*?);', data
                        )  # if we got "proxy-nossl.js" file
                        if proxy_groups:
                            proxy_server = proxy_groups.group(1)
                            proxy_scheme = "http://"
                    if not proxy_groups:
                ...
                try:
                    ...
                    for dmn in domains:
                        for dcnt in domains[dmn]:
                            ...
                            if len(leftover) < dmnl:
                                error_message = "Warning: Decompression did not yield enough data for domains['{}']['{}'] (needed {}, got {}). Storing partial data.".format(
                                    dmn, dcnt, dmnl, len(leftover)
                                )
                                log.warning(error_message)
                    ...
                    error_message = "Initializing Antizapret config failed: %s" % (
                        repr(e)
                    )
                    log.error(error_message)
                    raise Exception(error_message)
...
class AntizapretProxy(object):
    ...
    def detect(self, host=""):
        ...
        curarr = []
        if (
            curzone in self.config["domains"]
            and len(curhost) in self.config["domains"][curzone]
        ):
